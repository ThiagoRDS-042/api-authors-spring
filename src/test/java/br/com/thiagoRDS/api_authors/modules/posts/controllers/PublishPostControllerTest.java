...
public class PublishPostControllerTest {
  private MockMvc mvc;
  ...
  public void publishPost() throws Exception {
    ...
    this.mvc.perform(patch("/posts/" + post.getId() + "/publish")
        .header("Authorization", "Bearer " + response.token()))
        .andExpect(status().isNoContent());

    Optional<Post> responsePost = this.postsRepository.findById(post.getId());

    assertThat(responsePost.get().getPublishedAt()).isInstanceOf(LocalDateTime.class);
  }
}
